...
--	 They may not take courses simultaneously if their combined points total exceeds 180 points.
	...
	create function enrolmentt_constraint_1 (
		@studentID int,
		@courseID int
	)
	returns varchar(10)
	...
	ADD CONSTRAINT enrolmentt_constraint_2 CHECK(dbo.enrolmentt_constraint_1(studentID, CourseID) = 'True');
...
--2. The student's region and the counselor's region must be the same.
	...
	ALTER TABLE Staff
	ADD CONSTRAINT check_region_1 CHECK(dbo.check_region() = 0);

--3.

--- Check Credit

	create function check_credit_1 (
		@Title varchar(256),
		@Credit int
	)
	returns varchar(10)
	...
	ALTER TABLE Course
	ADD CONSTRAINT check_credit_2 CHECK(dbo.check_credit_1(Title, Credit) = 'True');
...
--1. Test the credit limit constraint.
	...
	insert into Course values('Math',25)
...
--2. Test that you have correctly defined the constraint for the student counsel's region. 
	...
	update Student set RegionID = 5 where StudentID=1

 /*
 Msg 547, Level 16, State 0, Line 65
The UPDATE statement conflicted with the FOREIGN KEY constraint "FK__Staff__RegionID__3C69FB99". The conflict occurred in database "UNIVERSITY", table "dbo.Region", column 'RegionID'.
The statement has been terminated.
 */
...
/*
Msg 547, Level 16, State 0, Line 91
The UPDATE statement conflicted with the CHECK constraint "check_credit_2". The conflict occurred in database "UNIVERSITY", table "dbo.Course".
The statement has been terminated.
*/
...
--6. Try to set Tom Garden as counsel of Alec Hunter (You should get an error.)
	...
	update Student set StaffID = 7 where StudentID=1
...
--	 If you get an error, read the error and update the reference rules for the relevant foreign key.
 ...
 -- update the reference rules for the relevant foreign key
